...
Important: Do not send processed one-hot or model-processed features; send raw profile and scenario fields only.
...
- `user_id`
- `profile_id`
- `assessment_id` — currently not persisted/generated (may be null)
- `warnings` — array
- `errors` — array
...
- `assessment_id` is currently not persisted/generated.
